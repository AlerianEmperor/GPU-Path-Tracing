...
void fix_normals(void)
{
	for (unsigned j = 0; j<g_trianglesNo; j++) {
		vec3 worldPointA = g_vertices[g_triangles[j]._idx1];
		vec3 worldPointB = g_vertices[g_triangles[j]._idx2];
		vec3 worldPointC = g_vertices[g_triangles[j]._idx3];
		vec3 AB = worldPointB;
		AB -= worldPointA;
		vec3 AC = worldPointC;
		AC -= worldPointA;
...
float load_object(const char *filename)
{
				...
				if (!inside) {
					...
					if (totalVertices) {
						...
						if (str2 >> dummy >> idx1 >> idx2 >> idx3)
						{
							...
							pCurrentTriangle->_normal = vec3(0, 0, 0);
							pCurrentTriangle->_bottom = vec3(FLT_MAX, FLT_MAX, FLT_MAX);
							pCurrentTriangle->_top = vec3(-FLT_MAX, -FLT_MAX, -FLT_MAX);
							Vertex *vertexA = &g_vertices[idx1];
							Vertex *vertexB = &g_vertices[idx2];
							Vertex *vertexC = &g_vertices[idx3];
							pCurrentTriangle->_center = vec3(
	...
	for (unsigned i = 0; i<g_trianglesNo; i++) {
		...
		vec3 vc1 = g_vertices[triangle._idx2] - g_vertices[triangle._idx1];
		vec3 vc2 = g_vertices[triangle._idx3] - g_vertices[triangle._idx2];
		vec3 vc3 = g_vertices[triangle._idx1] - g_vertices[triangle._idx3];
