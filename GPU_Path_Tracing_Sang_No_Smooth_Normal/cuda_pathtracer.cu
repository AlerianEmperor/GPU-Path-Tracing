...
__device__ bool hit_triangle(float3& origin, float3& direction, int& triangle_ind, float& mint)//, float4& n)
{
	float4 p0 = tex1Dfetch(triangle_values_texture, 4 * triangle_ind);
	float4 e1 = tex1Dfetch(triangle_values_texture, 4 * triangle_ind + 1);
	float4 e2 = tex1Dfetch(triangle_values_texture, 4 * triangle_ind + 2);
...
__device__ bool BVH_IntersectTriangles(float3& origin, float3& ray, int& triangle_ind, float& hit_t, int avoidSelf)
{
...
__device__ vec3 path_trace(curandState *randstate, vec3& originInWorldSpace, vec3& rayInWorldSpace, int avoidSelf)
{
	...
	for (int bounces = 0; bounces < 5; bounces++){  // iteration up to 4 bounces (instead of recursion in CPU code)
		...
		//float cache = 1e20;
		BVH_IntersectTriangles(rayorig, raydir, pBestTriIdx, triangle_t, avoidSelf);
...
int* cuda_bvh_indices, unsigned framenumber, unsigned hashedframes, Camera* cudaRendercam)
{
	...
	if (g_bFirstTime) {
		...
		cudaChannelFormatDesc channel2desc = cudaCreateChannelDesc<int1>();
