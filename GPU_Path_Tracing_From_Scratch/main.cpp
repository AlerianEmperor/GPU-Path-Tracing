...
void disp(void)
{
	...
	// gateway from host to CUDA, passes all data needed to render frame (triangles, BVH tree, camera) to CUDA for execution
	renderer(finaloutputbuffer, accumulatebuffer, framenumber, hashedframes);
...
/*void prepCUDAscene() {
	...
	for (unsigned f = 0; f<g_verticesNo; f++) {
		...
		pVerticesData[f * 8 + 0] = g_vertices[f].p.x;
		pVerticesData[f * 8 + 1] = g_vertices[f].p.y;
		pVerticesData[f * 8 + 2] = g_vertices[f].p.z;
		pVerticesData[f * 8 + 3] = 0.0f;//g_vertices[f]._ambientOcclusionCoeff;
										// second float4 stores vertex normal xyz
		pVerticesData[f * 8 + 4] = g_vertices[f].n.x;//_normal.x;
		pVerticesData[f * 8 + 5] = g_vertices[f].n.y;//_normal.y;
		pVerticesData[f * 8 + 6] = g_vertices[f].n.z;//_normal.z;
		pVerticesData[f * 8 + 7] = 0.0f;
	}
	...
	float *pTrianglesIntersectionData = (float *)malloc(g_trianglesNo * 16 * sizeof(float));

	for (unsigned e = 0; e<g_trianglesNo; e++) {
		// Texture-wise:
		//
		// first float4, triangle center + two-sided bool
		pTrianglesIntersectionData[16 * e + 0] = g_triangles[e].p0.x;//._center.x;
		pTrianglesIntersectionData[16 * e + 1] = g_triangles[e].p0.y;//_center.y;
		pTrianglesIntersectionData[16 * e + 2] = g_triangles[e].p0.z;//_center.z;
		pTrianglesIntersectionData[16 * e + 3] = 0.0f;//g_triangles[e]._twoSided ? 1.0f : 0.0f;
													  // second float4, normal
		pTrianglesIntersectionData[16 * e + 4] = g_triangles[e].n.x;//_normal.x;
		pTrianglesIntersectionData[16 * e + 5] = g_triangles[e].n.y;//_normal.y;
		pTrianglesIntersectionData[16 * e + 6] = g_triangles[e].n.z;//_normal.z;
		pTrianglesIntersectionData[16 * e + 7] = 0.0f;//g_triangles[e]._d;
													  // third float4, precomputed plane normal of triangle edge 1
		pTrianglesIntersectionData[16 * e + 8] = g_triangles[e].e1.x;//._e1.x;  
		pTrianglesIntersectionData[16 * e + 9] = g_triangles[e].e1.y;//_e1.y;
		pTrianglesIntersectionData[16 * e + 10] = g_triangles[e].e1.z;//_e1.z;
		pTrianglesIntersectionData[16 * e + 11] = 0.0f;//g_triangles[e]._d1;
													   // fourth float4, precomputed plane normal of triangle edge 2
		pTrianglesIntersectionData[16 * e + 12] = g_triangles[e].e2.x;//_e2.x; 
		pTrianglesIntersectionData[16 * e + 13] = g_triangles[e].e2.y;//_e2.y;
		pTrianglesIntersectionData[16 * e + 14] = g_triangles[e].e2.z;//_e2.z;
		pTrianglesIntersectionData[16 * e + 15] = 0.0f;//g_triangles[e]._d2;
													   
	}

	// copy precomputed triangle intersection data to CUDA global memory
	cudaMalloc((void**)&cudaTriangleIntersectionData2, g_trianglesNo * 16 * sizeof(float));
	cudaMemcpy(cudaTriangleIntersectionData2, pTrianglesIntersectionData, g_trianglesNo * 16 * sizeof(float), cudaMemcpyHostToDevice);
