...
BVHNode *Recurse(BBoxEntries& work, REPORTPRM(float pct = 0.) int depth = 0)
{
	...
	vec3 bottom(FLT_MAX, FLT_MAX, FLT_MAX);
	vec3 top(-FLT_MAX, -FLT_MAX, -FLT_MAX);
	...
	for (int j = 0; j < 3; j++) {  // 0 = X, 1 = Y, 2 = Z axis
		...
		for (float testSplit = start + step; testSplit < stop - step; testSplit += step) {
			...
			vec3 lbottom(FLT_MAX, FLT_MAX, FLT_MAX);
			vec3 ltop(-FLT_MAX, -FLT_MAX, -FLT_MAX);

			vec3 rbottom(FLT_MAX, FLT_MAX, FLT_MAX);
			vec3 rtop(-FLT_MAX, -FLT_MAX, -FLT_MAX);
