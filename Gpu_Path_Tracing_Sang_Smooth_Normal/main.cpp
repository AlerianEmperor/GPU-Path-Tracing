...
void disp(void)   
{
	...
	cudarender(finaloutputbuffer, accumulatebuffer, cuda_triangle_values, cuda_triangle_indices, cuda_bvh_boxes,
		cuda_bvh_indices, cuda_vertex_normals, framenumber, hashedframes, cudaRendercam2);
...
void prepCUDAScene2()
{
	...
	float* triangle_values = (float*)malloc(num_triangles * 12 * sizeof(float));
	int* triangle_indices = (int*)malloc(num_triangles * 4 * sizeof(int));

	for (int i = 0; i < num_triangles; ++i)
	{
		triangle_values[12 * i] = trs[i].p0.x;
		triangle_values[12 * i + 1] = trs[i].p0.y;
		triangle_values[12 * i + 2] = trs[i].p0.z;
		triangle_values[12 * i + 3] = 0.0f;

		triangle_values[12 * i + 4] = trs[i].e1.x;
		triangle_values[12 * i + 5] = trs[i].e1.y;
		triangle_values[12 * i + 6] = trs[i].e1.z;
		triangle_values[12 * i + 7] = 0.0f;

		triangle_values[12 * i + 8] = trs[i].e2.x;
		triangle_values[12 * i + 9] = trs[i].e2.y;
		triangle_values[12 * i + 10] = trs[i].e2.z;
		triangle_values[12 * i + 11] = 0.0f;
	...
	cudaMalloc((void**)&cuda_triangle_values, num_triangles * 12 * sizeof(float));
	cudaMemcpy(cuda_triangle_values, triangle_values, num_triangles * 12 * sizeof(float), cudaMemcpyHostToDevice);
	...
	float* vertex_normals = new float[num_vertices * 4];

	for (int i = 0; i < num_vertices; ++i)
	{
		vertex_normals[4 * i] = vts[i].n.x;
		vertex_normals[4 * i + 1] = vts[i].n.y;
		vertex_normals[4 * i + 2] = vts[i].n.z;
		vertex_normals[4 * i + 3] = 0.0f;
	}

	cudaMalloc((void**)&cuda_vertex_normals, num_vertices * 4 * sizeof(float));
	cudaMemcpy(cuda_vertex_normals, vertex_normals, num_vertices * 4 * sizeof(float), cudaMemcpyHostToDevice);
}
